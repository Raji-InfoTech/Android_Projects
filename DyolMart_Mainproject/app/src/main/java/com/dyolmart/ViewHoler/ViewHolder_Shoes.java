...
public class ViewHolder_Shoes extends RecyclerView.Adapter<ViewHolder_Shoes.myview> {
    ArrayList<products> list;


    public ViewHolder_Shoes(ArrayList<products> list) {
        this.list = list;
    }
    ...
    public void onBindViewHolder(@NonNull myview holder, int position) {
        ...
        // holder.t5.setText( list.get( position ).getCommonid());
        Glide.with(holder.img.getContext()).load(list.get(position).getImg_url1()).into(holder.img);

        holder.view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(v.getContext(), com.dyolmart.FindnextPage.Findnextpage_Shoes.class);
